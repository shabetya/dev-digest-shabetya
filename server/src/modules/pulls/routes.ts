...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // JS grouping is cheap per lookup.
    ...
    const countsByReviewId = new Map<string, SeverityCounts>();
    ...
    if (reviewIds.length > 0) {
      ...
      for (const f of findingRows) {
        const counts =
          countsByReviewId.get(f.reviewId) ?? { CRITICAL: 0, WARNING: 0, SUGGESTION: 0 };
        if (f.severity === 'CRITICAL' || f.severity === 'WARNING' || f.severity === 'SUGGESTION') {
          counts[f.severity] += 1;
        }
        countsByReviewId.set(f.reviewId, counts);
      }
    }
